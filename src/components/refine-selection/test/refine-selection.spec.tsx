...
import { ISelectionSet, EWorkflowType, ESelectionMode } from '../../../utils/interfaces';
  ...
  */
  ...
  it('selectionSetsChanged NO ids', async () => {
    const page = await newSpecPage({
      ...
      template: () => (
        <refine-selection
          selectionSets={[refineSelectionSet]}
        ></refine-selection>
      ),
    });

    page.doc.addEventListener("selectionSetsChanged", (evt: CustomEvent) => {
  ...
  it('selectionSetsChanged WITH ids', async () => {
    ...
    const page = await newSpecPage({
      ...
      template: () => (
        <refine-selection
          selectionSets={[refineSelectionSet, otherSelectionSetToRemoveFrom]}
        ></refine-selection>
      ),
    });
    ...
    page.doc.addEventListener("selectionSetsChanged", async (evt: CustomEvent) => {
      await page.waitForChanges();
  ...
  it('will add REFINE set', async () => {
    ...
    const page = await newSpecPage({
      ...
      template: () => (
        <refine-selection
          selectionSets={[otherSelectionSetToRemoveFrom]}
        ></refine-selection>
      ),
    });
