 ...
 *
 */
exports.ESeverity = void 0;
(function (ESeverity) {
    ESeverity["H"] = "H";
    ESeverity["M"] = "M";
    ESeverity["L"] = "L";
})(exports.ESeverity || (exports.ESeverity = {}));
/**
 * Type of distress as defined by ASTM standard
 *
 */
exports.EDistressType = void 0;
...
function calculateDeductValue(type, severity, density, showDebugging = false) {
    ...
    const _severity = exports.ESeverity[severity];
    ...
    let calc;
    switch (_type) {
        case exports.EDistressType.ALLIGATOR_CRACKING:
            calc = _calcAlligator;
            break;
        case exports.EDistressType.BLEEDING:
            calc = _calcBleeding;
            break;
        case exports.EDistressType.BLOCK_CRACKING:
            calc = _calcBlockCracking;
            break;
        case exports.EDistressType.BUMPS_SAGS:
            calc = _calcBumpsSags;
            break;
        case exports.EDistressType.CORRUGATION:
            calc = _calcCorrugation;
            break;
        case exports.EDistressType.DEPRESSION:
            calc = _calcDepression;
            break;
        case exports.EDistressType.EDGE_CRACKING:
            calc = _calcEdgeCracking;
            break;
        case exports.EDistressType.REFLECTION_CRACKING:
            calc = _calcReflectionCracking;
            break;
        case exports.EDistressType.LANE_SHOULDER_DROP_OFF:
            calc = _calcLaneShoulderDropOff;
            break;
        case exports.EDistressType.LONG_TRANS_CRACKING:
            calc = _calcLongTransCracking;
            break;
        case exports.EDistressType.PATCHING_UTIL_CUT_PATCHING:
            calc = _calcPatchingUtilCutPatching;
            break;
        case exports.EDistressType.POLISHED_AGGREGATE:
            calc = _calcPolishedAggregate;
            break;
        case exports.EDistressType.POTHOLES:
            calc = _calcPotholes;
            break;
        case exports.EDistressType.RAILROAD_CROSSING:
            calc = _calcRailroadCrossing;
            break;
        case exports.EDistressType.RUTTING:
            calc = _calcRutting;
            break;
        case exports.EDistressType.SHOVING:
            calc = _calcShoving;
            break;
        case exports.EDistressType.SLIPPAGE_CRACKING:
...
function _calcAlligator(severity, density) {
    const vals = severity === exports.ESeverity.H ? [30.71, 29.22, 9.407, -6.981, -2.158, 3.047, -0.6578] :
        severity === exports.ESeverity.M ? [21.62, 21.32, 5.194, -1.343, 0.2341] :
...
function _calcBleeding(severity, density) {
    const vals = severity === exports.ESeverity.H ? [5.343, 6.927, 7.926, 4.265, -0.7582] :
        severity === exports.ESeverity.M ? [2.38, 5.483, 4.128, 0.8366, 0.03659, 0.1052] :
...
function _calcBlockCracking(severity, density) {
    const vals = severity === exports.ESeverity.H ? [7.926, 11.93, 8.826, 3.432, -1.382] :
        severity === exports.ESeverity.M ? [2.587, 9.142, 6.647, -0.455, -0.2439, 0.1107] :
...
function _calcBumpsSags(severity, density) {
    const vals = severity === exports.ESeverity.H ? [52.77, 38.07, 8.964, -0.4148, -1.589, 2.478] :
        severity === exports.ESeverity.M ? [24.66, 27.86, 15.73, 3.295, -5.27, 5.921, 7.959] :
...
function _calcCorrugation(severity, density) {
    const vals = severity === exports.ESeverity.H ? [34.21, 21.59, 1.863, 6.1410, -0.687, -1.885, 0.4916] :
        severity === exports.ESeverity.M ? [15.78, 19.69, 6.276, -2.124, 0.5868, 0.06045] :
...
function _calcDepression(severity, density) {
    const vals = severity === exports.ESeverity.H ? [17.84, 5.904, 9.406, 18.28, -2.753, -7.798, 2.402] :
        severity === exports.ESeverity.M ? [8.471, 3.171, 10.25, 12.2, -0.9687, -4.601, 1.079] :
...
function _calcEdgeCracking(severity, density) {
    const vals = severity === exports.ESeverity.H ? [14.99, 13.5, 10.62, 5.15, -1.792, -2.826] :
        severity === exports.ESeverity.M ? [9.177, 10.26, 5.704, -0.6812, -0.8588, 0.2123] :
...
function _calcReflectionCracking(severity, density) {
    const vals = severity === exports.ESeverity.H ? [14.99, 17.21, 27.63, 31.13, -20.94, -18.82, 9.985] :
        severity === exports.ESeverity.M ? [9.9193, 10.88, 8.073, 14.8, -3.792, -9.583, 3.651] :
...
function _calcLaneShoulderDropOff(severity, density) {
    const vals = severity === exports.ESeverity.H ? [7.549, 6.907, 7.284, 9.304, 2.064] :
        severity === exports.ESeverity.M ? [4.02, 1.744, 14.36, 15.92, -40.22, 23.6] :
...
function _calcLongTransCracking(severity, density) {
    const vals = severity === exports.ESeverity.H ? [19.56, 24.06, 12.12, 9.336, 1.532, -4.396] :
        severity === exports.ESeverity.M ? [9.751, 15.53, 4.719, 1.369, 1.206, -1.164] :
...
function _calcPatchingUtilCutPatching(severity, density) {
    const vals = severity === exports.ESeverity.H ? [20.15, 16.55, 5.415, 10.77, 3.257, -4.502] :
        severity === exports.ESeverity.M ? [10.28, 12.71, 6.174, 1.928, 0.6923, -0.4673] :
...
function _calcPolishedAggregate(severity, density) {
    const vals = severity === exports.ESeverity.H || severity === exports.ESeverity.M || severity === exports.ESeverity.L ?
...
function _calcPotholes(severity, density) {
    const vals = severity === exports.ESeverity.H ? [110.7, 60.66, 2.378, -2.664] :
        severity === exports.ESeverity.M ? [90.47, 60.41, -0.1123, -4.746] :
...
function _calcRailroadCrossing(severity, density) {
    const vals = severity === exports.ESeverity.H ? [20, 38.64, -62.82, 193.6, -162.7, 41.22] :
        severity === exports.ESeverity.M ? [6.994, 23.47, -52.3, 167.3, -143.6, 37.64] :
...
function _calcRutting(severity, density) {
    const vals = severity === exports.ESeverity.H ? [27.61, 25.19, 8.557, 1.65, -2.2030] :
        severity === exports.ESeverity.M ? [18.47, 20.77, 6.617, -1.13, -2.286] :
...
function _calcShoving(severity, density) {
    const vals = severity === exports.ESeverity.H ? [18.7, 20.17, 10.3, 4.694, 1.331, -2.61] :
        severity === exports.ESeverity.M ? [10.39, 14.78, 5.488, 4.001, 3.23, -2.387] :
...
function _calcSlippageCracking(severity, density) {
    const vals = severity === exports.ESeverity.H ? [19.73, 32.21, 25.87, -2.871, -10.55, 1.496, 0.7393] :
        severity === exports.ESeverity.M ? [11.38, 18.55, 18.38, 1.628, -7.596, 0.5841, 0.5946] :
...
function _calcSwell(severity, density) {
    const vals = severity === exports.ESeverity.H ? [34, 16.84, -23.09, 41.17, -14.84] :
        severity === exports.ESeverity.M ? [12, 15.67, 20.59, -28.33, 21, -5.508] :
