...
const index$1 = require('./index-6654298b.js');
const index = require('./index-951f09d7.js');
const dom = require('./dom-24094fab.js');
const utils = require('./utils-08e4a820.js');
...
const throttle = require('./throttle-8ee74c77.js');
...
const defaultValue = utils.normalizeHex(DEFAULT_COLOR.hex());
const defaultFormat = "auto";
const ColorPicker = class {
  constructor(hostRef) {
    index$1.registerInstance(this, hostRef);
    this.calciteColorPickerChange = index$1.createEvent(this, "calciteColorPickerChange", 6);
    this.calciteColorPickerInput = index$1.createEvent(this, "calciteColorPickerInput", 6);
    ...
    this.handleHexInputChange = (event) => {
      ...
      if (hex !== normalizedHex) {
        this.internalColorSet(index.color(hex));
      }
    };
    this.handleSavedColorSelect = (event) => {
      const swatch = event.currentTarget;
      this.internalColorSet(index.color(swatch.color));
    };
    ...
    this.renderChannelsTabTitle = (channelMode) => {
      ...
      return (index$1.h("calcite-tab-title", { class: CSS.colorMode, "data-color-mode": channelMode, key: channelMode, onCalciteTabsActivate: this.handleTabActivate, selected: selected }, label));
    };
    this.renderChannelsTab = (channelMode) => {
      ...
      return (index$1.h("calcite-tab", { class: CSS.control, key: channelMode, selected: selected }, index$1.h("div", { class: CSS.channels, dir: "ltr" }, channels.map((channel, index) => 
      /* the channel container is ltr, so we apply the host's direction */
      this.renderChannel(channel, index, channelLabels[index], channelAriaLabels[index], direction)))));
    };
    this.renderChannel = (value, index, label, ariaLabel, direction) => (index$1.h("calcite-input", { class: CSS.channel, "data-channel-index": index, dir: direction, label: ariaLabel, lang: this.effectiveLocale, numberButtonType: "none", numberingSystem: this.numberingSystem, onCalciteInputChange: this.handleChannelChange, onCalciteInputInput: this.handleChannelInput, onKeyDown: this.handleKeyDown, prefixText: label, scale: this.scale === "l" ? "m" : "s", type: "number", value: value === null || value === void 0 ? void 0 : value.toString() }));
    this.deleteColor = () => {
  ...
  handleValueChange(value, oldValue) {
    ...
    const color = allowEmpty && !value ? null : index.color(value);
  ...
  async componentWillLoad() {
    ...
    const initialColor = willSetNoColor ? null : valueIsCompatible ? index.color(value) : color;
  ...
  render() {
    ...
    return (index$1.h("div", { class: CSS.container }, index$1.h("div", { class: CSS.colorFieldAndSliderWrap }, index$1.h("canvas", { class: {
      ...
      ref: this.initColorFieldAndSlider }), index$1.h("div", { "aria-label": vertical ? messages.value : messages.saturation, "aria-valuemax": vertical ? HSV_LIMITS.v : HSV_LIMITS.s, "aria-valuemin": "0", "aria-valuenow": (vertical ? color === null || color === void 0 ? void 0 : color.saturationv() : color === null || color === void 0 ? void 0 : color.value()) || "0", class: { [CSS.scope]: true, [CSS.colorFieldScope]: true }, onKeyDown: this.handleColorFieldScopeKeyDown, role: "slider", style: { top: `${colorFieldScopeTop || 0}px`, left: `${colorFieldScopeLeft || 0}px` }, tabindex: "0",
      // eslint-disable-next-line react/jsx-sort-props
      ref: this.storeColorFieldScope }), index$1.h("div", { "aria-label": messages.hue, "aria-valuemax": HSV_LIMITS.h, "aria-valuemin": "0", "aria-valuenow": (color === null || color === void 0 ? void 0 : color.round().hue()) || DEFAULT_COLOR.round().hue(), class: { [CSS.scope]: true, [CSS.hueScope]: true }, onKeyDown: this.handleHueScopeKeyDown, role: "slider", style: { top: `${hueTop}px`, left: `${hueLeft}px` }, tabindex: "0",
      // eslint-disable-next-line react/jsx-sort-props
      ref: this.storeHueScope })), hideHex && hideChannels ? null : (index$1.h("div", { class: {
        [CSS.controlSection]: true,
        [CSS.section]: true
      } }, hideHex ? null : (index$1.h("div", { class: CSS.hexOptions }, index$1.h("span", { class: {
        [CSS.header]: true,
        [CSS.headerHex]: true
      } }, messages.hex), index$1.h("calcite-color-picker-hex-input", { allowEmpty: allowEmpty, class: CSS.control, hexLabel: messages.hex, numberingSystem: this.numberingSystem, onCalciteColorPickerHexInputChange: this.handleHexInputChange, scale: hexInputScale, value: selectedColorInHex }))), hideChannels ? null : (index$1.h("calcite-tabs", { class: {
        [CSS.colorModeContainer]: true,
        [CSS.splitSection]: true
      }, scale: hexInputScale }, index$1.h("calcite-tab-nav", { slot: "title-group" }, this.renderChannelsTabTitle("rgb"), this.renderChannelsTabTitle("hsv")), this.renderChannelsTab("rgb"), this.renderChannelsTab("hsv"))))), hideSaved ? null : (index$1.h("div", { class: { [CSS.savedColorsSection]: true, [CSS.section]: true } }, index$1.h("div", { class: CSS.header }, index$1.h("label", null, messages.saved), index$1.h("div", { class: CSS.savedColorsButtons }, index$1.h("calcite-button", { appearance: "transparent", class: CSS.deleteColor, disabled: noColor, iconStart: "minus", kind: "neutral", label: messages.deleteColor, onClick: this.deleteColor, scale: hexInputScale, type: "button" }), index$1.h("calcite-button", { appearance: "transparent", class: CSS.saveColor, disabled: noColor, iconStart: "plus", kind: "neutral", label: messages.saveColor, onClick: this.saveColor, scale: hexInputScale, type: "button" }))), savedColors.length > 0 ? (index$1.h("div", { class: CSS.savedColors }, [
      ...savedColors.map((color) => (index$1.h("calcite-color-picker-swatch", { active: selectedColorInHex === color, class: CSS.savedColor, color: color, key: color, onClick: this.handleSavedColorSelect, onKeyDown: this.handleSavedColorKeyDown, scale: scale, tabIndex: 0 })))
    ])) : null))));
  }
  // --------------------------------------------------------------------------
  //
  ...
  drawHueSlider() {
    ...
    hueSliderColorStopKeywords.forEach((keyword) => {
      gradient.addColorStop(currentOffset, index.color(keyword).string());
  ...
  updateColorFromChannels(channels) {
    this.internalColorSet(index.color(channels, this.channelMode));
  }
