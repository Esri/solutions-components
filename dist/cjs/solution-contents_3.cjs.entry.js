...
const common = require('./common-4d4cf291.js');
...
const SolutionSpatialRef = class {
    ...
    solutionStoreHasChanges() {
        this.featureServices = solutionStore.state.getStoreInfo("featureServices");
    }
    //--------------------------------------------------------------------------
    //
    //  Public Methods (async)
    ...
    _parameterizeWkid(wkid) {
        return wkid
            ? wkid.toString().startsWith(interfaces.CSpatialRefCustomizingPrefix)
                ? wkid
                : `${interfaces.CSpatialRefCustomizingPrefix}${wkid}${interfaces.CSpatialRefCustomizingSuffix}`
            : wkid;
    }
    ;
    /**
    ...
    _renderFeatureServicesList(featureServices) {
        ...
        return configurableServices.length > 0 ? (index.h("div", null, index.h("label", { class: "spatial-ref-item-title" }, this._translations.featureServicesHeading), index.h("ul", { class: "spatial-ref-services-list" }, configurableServices.map(configurableService => (index.h("li", { class: "spatial-ref-services-list-item" }, index.h("label", { class: "switch-label" }, index.h("calcite-switch", { checked: spatialReferenceInfo.services[configurableService.id], class: "spatial-ref-item-switch", disabled: !this.enabled, onCalciteSwitchChange: (event) => this._updateEnabledServices(event, configurableService), scale: "m" }), configurableService.name))))))) : (null);
    }
    /**
    ...
    _unparameterizeWkid(wkid) {
        return wkid && wkid.toString().startsWith(interfaces.CSpatialRefCustomizingPrefix)
            ? wkid.substring(interfaces.CSpatialRefCustomizingPrefix.length, wkid.length - interfaces.CSpatialRefCustomizingSuffix.length)
            : wkid;
    }
    /**
    ...
    _updateFeatureServices(spatialReferenceInfo) {
        ...
        solutionStore.state.setStoreInfo("featureServices", this.featureServices);
    }
    /**
