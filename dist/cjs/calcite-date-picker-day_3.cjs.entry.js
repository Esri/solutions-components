...
const DatePickerDay = class {
  ...
  render() {
    const dayId = date.dateToISO(this.value).replaceAll("-", "");
    if (this.parentDatePickerEl) {
      const { numberingSystem, lang: locale$1 } = this.parentDatePickerEl;
      locale.numberStringFormatter.numberFormatOptions = Object.assign(Object.assign({ useGrouping: false }, (numberingSystem && { numberingSystem })), (locale$1 && { locale: locale$1 }));
    }
    const formattedDay = locale.numberStringFormatter.localize(String(this.day));
    const dir = dom.getElementDir(this.el);
    return (index.h(index.Host, { id: dayId, onClick: this.onClick, onKeyDown: this.keyDownHandler, role: "gridcell" }, index.h("div", { class: { "day-v-wrapper": true, [resources.CSS_UTILITY.rtl]: dir === "rtl" } }, index.h("div", { class: "day-wrapper" }, index.h("span", { class: "day" }, index.h("span", { class: "text" }, formattedDay))))));
  }
  componentDidRender() {
...
const datePickerMonthCss = "@keyframes in{0%{opacity:0}100%{opacity:1}}@keyframes in-down{0%{opacity:0;transform:translate3D(0, -5px, 0)}100%{opacity:1;transform:translate3D(0, 0, 0)}}@keyframes in-up{0%{opacity:0;transform:translate3D(0, 5px, 0)}100%{opacity:1;transform:translate3D(0, 0, 0)}}@keyframes in-scale{0%{opacity:0;transform:scale3D(0.95, 0.95, 1)}100%{opacity:1;transform:scale3D(1, 1, 1)}}:root{--calcite-animation-timing:calc(150ms * var(--calcite-internal-duration-factor));--calcite-internal-duration-factor:var(--calcite-duration-factor, 1);--calcite-internal-animation-timing-fast:calc(100ms * var(--calcite-internal-duration-factor));--calcite-internal-animation-timing-medium:calc(200ms * var(--calcite-internal-duration-factor));--calcite-internal-animation-timing-slow:calc(300ms * var(--calcite-internal-duration-factor))}.calcite-animate{opacity:0;animation-fill-mode:both;animation-duration:var(--calcite-animation-timing)}.calcite-animate__in{animation-name:in}.calcite-animate__in-down{animation-name:in-down}.calcite-animate__in-up{animation-name:in-up}.calcite-animate__in-scale{animation-name:in-scale}@media (prefers-reduced-motion: reduce){:root{--calcite-internal-duration-factor:0.01}}:root{--calcite-floating-ui-transition:var(--calcite-animation-timing);--calcite-floating-ui-z-index:600}:host([hidden]){display:none}.calender{-webkit-margin-after:0.25rem;margin-block-end:0.25rem}.week-headers{display:flex;border-width:0px;border-block-start-width:1px;border-style:solid;border-color:var(--calcite-ui-border-3);padding-block:0px;padding-inline:0.25rem}.week-header{text-align:center;font-weight:var(--calcite-font-weight-bold);color:var(--calcite-ui-text-3);inline-size:14.2857142857%}:host([scale=s]) .week-header{padding-inline:0px;padding-block:0.5rem 0.75rem;font-size:var(--calcite-font-size--2);line-height:1rem}:host([scale=m]) .week-header{padding-inline:0px;padding-block:0.75rem 1rem;font-size:var(--calcite-font-size--2);line-height:1rem}:host([scale=l]) .week-header{padding-inline:0px;padding-block:1rem 1.25rem;font-size:var(--calcite-font-size--1);line-height:1rem}.week-days{display:flex;flex-direction:row;padding-block:0px;padding-inline:6px}.week-days:focus{outline:2px solid transparent;outline-offset:2px}";
...
const DatePickerMonth = class {
  ...
  render() {
    ...
    const days = [
      ...
      ...curMonDays.map((day) => {
        ...
        return this.renderDateDay(active, day, date$1, true, true);
      }),
      ...nextMonDays.map((day) => {
        const date = new Date(year, month + 1, day);
        return this.renderDateDay(false, day, date);
  ...
  addMonths(step) {
    ...
    this.calciteInternalDatePickerActiveDateChange.emit(date.dateFromRange(nextDate, this.min, this.max));
   ...
   * @param ref
   */
  renderDateDay(active, day, date$1, currentMonth, ref) {
    var _a;
    ...
    return (index.h("calcite-date-picker-day", { active: active, class: {
      ...
      }, currentMonth: currentMonth, day: day, disabled: !date.inRange(date$1, this.min, this.max), endOfRange: this.isEndOfRange(date$1), highlighted: this.betweenSelectedRange(date$1), key: date$1.toDateString(), onCalciteDaySelect: this.daySelect, onCalciteInternalDayHover: this.dayHover, range: !!this.startDate && !!this.endDate && !date.sameDate(this.startDate, this.endDate), rangeHover: this.isRangeHover(date$1), scale: this.scale, selected: this.isSelected(date$1), startOfRange: this.isStartOfRange(date$1), value: date$1,
  ...
  isRangeHover(date$1) {
    ...
    const cond2 = !insideRange &&
      ((!isStart && date$1 >= this.endDate && (date$1 < end || date.sameDate(date$1, end))) ||
        (isStart &&
          ((this.startDate && date$1 < this.startDate) ||
            (this.endDate && date.sameDate(date$1, this.startDate))) &&
          ((start && date$1 > start) || date.sameDate(date$1, start))));
    return cond1 || cond2;
  }
  get el() { return index.getElement(this); }
};
...
const DatePickerMonthHeader = class {
  ...
  getInRangeDate({ localizedYear, offset = 0 }) {
    ...
    if (year && inRange && length === localizedYear.length) {
      ...
      return date.dateFromRange(nextDate, min, max);
    }
  }
  /**
