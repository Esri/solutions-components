...
const CardManager = class {
   ...
   */
    ...
    async _getFeaturesByIds(ids) {
        ...
        const featureSet = ids.length > 0 ? await mapViewUtils.queryFeaturesByID(ids, this.layer, [], true, this.mapView.spatialReference) : [];
...
const MapCard = class {
    ...
    _getDropdown(id) {
        const dropdownItems = this._getDropdownItems();
        return dropdownItems.length > 0 ? (index.h("calcite-dropdown", { closeOnSelectDisabled: true, disabled: this.selectedLayer === undefined, id: "solutions-more", onCalciteDropdownBeforeClose: () => this._forceShowHide(), ref: (el) => this._moreDropdown = el }, index.h("calcite-action", { appearance: "solid", id: id, label: "", onClick: () => this._closeShowHide(), slot: "trigger", text: "" }, index.h("calcite-button", { appearance: "transparent", iconEnd: "chevron-down", kind: "neutral" }, this._translations.more)), index.h("calcite-dropdown-group", { "selection-mode": "none" }, dropdownItems.map(item => {
     ...
     */
    _getToolTip(placement, referenceElement, text) {
        return document.getElementById(referenceElement) ? (index.h("calcite-tooltip", { placement: placement, "reference-element": "map-" + referenceElement }, index.h("span", null, text))) : undefined;
    }
    /**
     * Get a list of toolInfos that should display outside of the dropdown
     *
    ...
    _getAction(active, icon, indicator, label, func, disabled, loading, slot) {
        ...
        return (index.h("div", { class: "display-flex", id: this._getId(icon), slot: slot }, index.h("calcite-action", { active: active, appearance: "solid", disabled: _disabled, icon: icon, id: "map-" + icon, indicator: indicator, label: label, loading: loading, onClick: func, text: label, textEnabled: true }), this._getToolTip("bottom", icon, label)));
    }
    /**
     * Get an id with a prefix to the user supplied value
     *
