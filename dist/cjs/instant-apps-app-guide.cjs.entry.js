...
const InstantAppsAppGuide = class {
    ...
    render() {
        return (index.h(index.Host, { key: '3c6bf465e2543e62f8e991a22a4f8b8420556285' }, index.h("calcite-panel", { key: '15a145d4f930de4325716dcd8b73fdcbbe5fa31e', scale: "s" }, this._renderAppGuideHeader(), index.h("calcite-carousel", { key: '8f962b828f76b8a68f19b71fb79d3f4c93420e38', onCalciteCarouselChange: () => this._updateHeaderText(), ref: el => this._carouselRef = el, "arrow-type": this._getArrowType() }, this._renderAppGuidePages(this._viewModel.pages)))));
    }
    _updateHeaderText() {
        var _a;
    ...
    _renderAppGuideHeader() {
        ...
        return displayHeader ?
            (index.h("span", { slot: "header-content" }, _headerText, " ", index.h("calcite-icon", { icon: "lightbulb", scale: "s" }))) :
            null;
    }
    _renderAppGuidePages(pages) {
        return pages.map((pageData, index$1) => {
            const { content, type } = pageData;
            return (index.h("calcite-carousel-item", { key: `page_${index$1}` }, index.h("div", { class: CSS.contentWrapper }, this._renderContentItems(content, type))));
        });
    }
    _renderContentItems(content, type) {
        switch (type) {
            case 'list':
                return (index.h("ol", { class: CSS.contentList }, content.map((contentItem, index$1) => (index.h("li", { key: `content-list-item-${index$1}`, class: CSS.contentListItem }, contentItem)))));
            case 'paragraphs':
            default:
                return content.map((contentItem, index$1) => (index.h("p", { key: `content-paragraph-${index$1}` }, contentItem)));
        }
    }
...
exports.instant_apps_app_guide = InstantAppsAppGuide;
