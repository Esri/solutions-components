...
function getFeatureServices(templates) {
    return templates.reduce((prev, cur) => {
        ...
        if (cur.type === "Feature Service" &&
            ...
            prev.push({ id: cur.itemId, name, enabled: wkid.toString().startsWith(interfaces.CSpatialRefCustomizingPrefix), wkid });
        }
        return prev;
    }, []);
}
...
function getSpatialReferenceInfo(services, data) {
    ...
    const enabled = restHelpersGet.getPropWithDefault(data, "params.wkid.attributes.required", false);
    return wkid
        ? {
            enabled,
            default: wkid,
