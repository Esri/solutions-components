...
const InputText = class {
  ...
  render() {
    ...
    const loader = (index.h("div", { class: CSS$3.loader }, index.h("calcite-progress", { label: this.messages.loading, type: "indeterminate" })));
    const inputClearButton = (index.h("button", { "aria-label": this.messages.clear, class: CSS$3.clearButton, disabled: this.disabled || this.readOnly, onClick: this.clearInputTextValue, tabIndex: -1, type: "button" }, index.h("calcite-icon", { icon: "x", scale: this.scale === "l" ? "m" : "s" })));
    const iconEl = (index.h("calcite-icon", { class: CSS$3.inputIcon, flipRtl: this.iconFlipRtl, icon: this.requestedIcon, scale: this.scale === "l" ? "m" : "s" }));
    const prefixText = index.h("div", { class: CSS$3.prefix }, this.prefixText);
    const suffixText = index.h("div", { class: CSS$3.suffix }, this.suffixText);
    ...
    return (index.h(index.Host, { onClick: this.clickHandler, onKeyDown: this.keyDownHandler }, index.h("div", { class: { [CSS$3.inputWrapper]: true, [resources.CSS_UTILITY.rtl]: dir === "rtl" } }, this.prefixText ? prefixText : null, index.h("div", { class: CSS$3.wrapper }, childEl, this.isClearable ? inputClearButton : null, this.requestedIcon ? iconEl : null, this.loading ? loader : null), index.h("div", { class: CSS$3.actionWrapper }, index.h("slot", { name: SLOTS$2.action })), this.suffixText ? suffixText : null, index.h(form.HiddenFormInputSlot, { component: this }))));
  }
...
const List = class {
  ...
  render() {
    ...
    return (index.h("div", { class: CSS$2.container }, loading ? index.h("calcite-scrim", { class: CSS$2.scrim, loading: loading }) : null, index.h("table", { "aria-busy": dom.toAriaBoolean(loading), "aria-label": label || "", class: CSS$2.table, onKeyDown: this.handleListKeydown, role: "treegrid" }, filterEnabled ? (index.h("thead", null, index.h("tr", { class: { [CSS$2.sticky]: true } }, index.h("th", { colSpan: utils.MAX_COLUMNS }, index.h("calcite-filter", { "aria-label": filterPlaceholder, disabled: loading || disabled, items: dataForFilter, onCalciteFilterChange: this.handleFilter, placeholder: filterPlaceholder, value: filterText,
      // eslint-disable-next-line react/jsx-sort-props
      ref: (el) => (this.filterEl = el) }))))) : null, index.h("tbody", { class: CSS$2.tableContainer }, index.h("slot", { onSlotchange: this.handleDefaultSlotChange })))));
  }
  updateListItems() {
...
const Notice = class {
  ...
  render() {
    ...
    const hasActionEnd = dom.getSlotted(el, SLOTS$1.actionsEnd);
    return (index.h("div", { class: CSS$1.container }, this.requestedIcon ? (index.h("div", { class: CSS$1.icon }, index.h("calcite-icon", { flipRtl: this.iconFlipRtl, icon: this.requestedIcon, scale: this.scale === "l" ? "m" : "s" }))) : null, index.h("div", { class: CSS$1.content }, index.h("slot", { name: SLOTS$1.title }), index.h("slot", { name: SLOTS$1.message }), index.h("slot", { name: SLOTS$1.link })), hasActionEnd ? (index.h("div", { class: CSS$1.actionsEnd }, index.h("slot", { name: SLOTS$1.actionsEnd }))) : null, this.closable ? closeButton : null));
  }
  //--------------------------------------------------------------------------
  //
  //  Public Methods
