...
function getComponentClosestLanguage() {
    ...
    if (languageUtil.languageMap.has(lang)) {
        return languageUtil.languageMap.get(lang);
    }
    else {
        // "ru-RU" maps to "ru" use case
        if (languageUtil.languageMap.has(lang.slice(0, 2))) {
            return languageUtil.languageMap.get(lang.slice(0, 2));
        }
        else {
            return 'en';
        }
...
async function fetchLocaleStringsForComponent(componentName, locale) {
    ...
    try {
        return await fetchJson(IS_TEST_ENV ? fallbackURL : index.getAssetPath(localePath));
    }
    catch (primaryError) {
