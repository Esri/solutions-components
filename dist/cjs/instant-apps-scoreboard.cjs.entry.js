...
const MOBILE_BREAKPOINT = breakpoints.widthBreakpoints.medium[1];
const CSS = {
    BASE,
...
const InstantAppsScoreboard = class {
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.scoreboardItemsUpdated = index.createEvent(this, "scoreboardItemsUpdated", 7);
        this.view = undefined;
    ...
    async initMessages() {
        let messages;
        try {
            const res = await locale.getLocaleComponentStrings(this.el);
            messages = res[0];
    ...
    async initModules() {
        try {
            const [Handles, reactiveUtils, Collection, intl] = await loadModules.loadModules(['esri/core/Handles', 'esri/core/reactiveUtils', 'esri/core/Collection', 'esri/intl']);
    ...
    render() {
        ...
        return index.h(index.Host, { key: '0a4e0bd527aa63498f71650b418da1167c0570c0', class: `${positionClass} ${styleClass}` }, isDisabled ? null : [progress, ((_b = this.items) === null || _b === void 0 ? void 0 : _b.length) > 0 ? this.renderBase() : null]);
    }
    renderBase() {
    ...
    renderItemsContainer() {
        ...
        return (index.h("div", { class: CSS.itemsContainer }, previousButton, this.renderItems(), nextButton));
    }
    renderPreviousNextButtons() {
        ...
        return [
            index.h("calcite-action", { onClick: this.previousItem.bind(this), icon: previousIcon, disabled: isDisabled.previous, alignment: iconPosition, scale: scale, appearance: appearance, text: t9n.previous, label: t9n.previous }),
            index.h("calcite-action", { onClick: this.nextItem.bind(this), icon: nextIcon, disabled: isDisabled.next, alignment: iconPosition, scale: scale, appearance: appearance, text: t9n.next, label: t9n.next }),
        ];
    }
    renderItems() {
        const { items } = CSS;
        ...
        return index.h("ul", { class: items }, scoreboardItems);
    }
    renderItem(scoreboardItem) {
        const { label } = scoreboardItem;
        return (index.h("li", { class: CSS.item }, index.h("span", { class: CSS.label, title: label }, label), this.renderValue(scoreboardItem)));
    }
    renderValue(scoreboardItem) {
        ...
        const content = showPlaceholder ? (this.renderValuePlaceholder()) : !isDisabled ? (isNotVisible ? (index.h("calcite-icon", { icon: "view-hide", scale: "l", title: (_c = this.messages) === null || _c === void 0 ? void 0 : _c.layerVisibilityOff })) : (valueToDisplay)) : ('');
        return index.h("span", { class: CSS.value }, content);
    }
    renderValuePlaceholder() {
        return index.h("span", { class: CSS.valuePlaceholder });
    }
    renderProgress() {
        const key = `${KEY_PREFIX}calcite-`;
        return index.h("calcite-progress", { key: `${key}progress`, type: "indeterminate" });
    }
    renderLoader() {
        var _a;
        const key = `${KEY_PREFIX}calcite-`;
        const loading = (_a = this.messages) === null || _a === void 0 ? void 0 : _a.loading;
        return index.h("calcite-loader", { key: `${key}loader`, label: loading, text: loading, scale: "m" });
    }
...
InstantAppsScoreboard.style = InstantAppsScoreboardStyle0;

exports.instant_apps_scoreboard = InstantAppsScoreboard;
