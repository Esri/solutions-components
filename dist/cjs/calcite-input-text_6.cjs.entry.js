...
const downloadUtils = require('./downloadUtils-0ab0b5a6.js');
...
const PdfDownload = class {
  ...
  async downloadPDF(exportInfos, removeDuplicates) {
    Object.keys(exportInfos).forEach(k => {
      const exportInfo = exportInfos[k];
      void downloadUtils.downloadPDF(exportInfo.selectionSetNames, exportInfo.layerView.layer, exportInfo.ids, removeDuplicates, this._labelInfoElement.selectedOption.value);
    });
  }
  //--------------------------------------------------------------------------
  //
