...
const utils = require('./utils-ea7a5a63.js');
const loadModules = require('./loadModules-69fc5b98.js');
const locale = require('./locale-3e33f44c.js');
const languageSwitcher = require('./languageSwitcher-4822684b.js');
...
const InstantAppsLanguageTranslator = class {
    ...
    async initPortalItemResourceT9nData() {
        try {
            const portalItemResource = (await languageSwitcher.getPortalItemResource(this.portalItem));
            utils.store.set('portalItemResource', portalItemResource);
            const t9nData = await languageSwitcher.fetchResourceData(this.request, portalItemResource);
            utils.store.set('portalItemResourceT9n', t9nData !== null && t9nData !== void 0 ? t9nData : {});
        }
        catch (_a) { }
    }
    render() {
        return (index.h(index.Host, null, this.renderModal(), this.renderPopoverTip()));
    }
    renderModal() {
        return (index.h("calcite-modal", { open: this.open, scale: "l", fullscreen: true, onCalciteModalClose: this.close.bind(this) }, this.renderHeader(), this.renderContent(), this.renderPrimaryButton()));
    }
    renderPopoverTip() {
        var _a;
    ...
    renderHeader() {
        ...
        return (index.h("header", { class: CSS.header, slot: "header" }, this.renderHeaderText(), saving ? this.renderSavingIndicator() : null));
    }
    renderHeaderText() {
        const { messages } = this;
    ...
    renderSavingIndicator() {
        ...
        return (index.h("div", { class: CSS.savingIndicator }, saving ? index.h("calcite-loader", { label: t9n, inline: true }) : null, index.h("span", null, t9n)));
    }
    renderContent() {
    ...
    renderUserLocale() {
        ...
        const localeFlag = locale.getComponentClosestLanguage(this.el);
    ...
    renderUIDataItem(key, keyIndex, uiDataKeysLen) {
        ...
        return (index.h("instant-apps-language-translator-item", { key: `${key}-${keyIndex}`, class: isLast, fieldName: key, translatedLanguageLabels: translatedLanguageLabels, setting: setting, userLocaleInputOnChangeCallback: this.userLocaleInputOnChangeCallback, translatedLocaleInputOnChangeCallback: this.translatedLocaleInputOnChangeCallback }));
    }
    renderPrimaryButton() {
        var _a;
        return (index.h("calcite-button", { onClick: () => (this.open = false), slot: "primary", class: CSS.closeButton }, (_a = this.messages) === null || _a === void 0 ? void 0 : _a.close));
    }
