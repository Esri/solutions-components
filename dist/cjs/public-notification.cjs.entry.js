...
const PublicNotification = class {
    constructor(hostRef) {
        ...
        this._exportType = interfaces.EExportType.PDF;
    ...
    render() {
        ...
        return (index.h(index.Host, { key: 'fca8516bad668916b9fa1799a201ea61c032fdbc' }, index.h("calcite-shell", { key: 'aa43348a1f769eb0adf600b2bf130a6707ffd349' }, index.h("calcite-action-bar", { key: 'e3b6df1189a0bca8f7303465e08e03c633645b8d', class: "border-bottom-1 action-bar-size", "expand-disabled": true, layout: "horizontal", slot: headerSlot }, this._getActionGroup("list-check", interfaces.EPageType.LIST, this._translations.myLists), this.showRefineSelection ? this._getActionGroup("test-data", interfaces.EPageType.REFINE, this._translations.refineSelection) : null, this._getActionGroup("export", interfaces.EPageType.EXPORT, this._translations.export)), this._getPage(this._pageType))));
    }
    /**
     ...
     */
    _getOnboardingImage() {
        return (index.h("div", { class: "display-flex padding-sides-1" }, index.h("img", { class: "img-container", role: "presentation", src: this._onboardingImageUrl })));
    }
    /**
    ...
    _getSelectionSetList() {
        return (index.h("div", { class: "padding-top-1-2 padding-bottom-1-2" }, index.h("calcite-list", { class: "list-border margin-sides-1" }, this._selectionSets.reduce((prev, cur, i) => {
            ...
            if (validSet) {
                prev.push((index.h("calcite-list-item", { label: cur.label, onClick: () => this._gotoSelection(cur, this.mapView) }, index.h("div", { slot: "content" }, index.h("div", { class: "list-label" }, cur.label), index.h("div", { class: "list-description" }, cur?.layerView?.layer.title), index.h("div", { class: "list-description" }, this._translations.selectedFeatures.replace("{{n}}", ids.length.toString()))), this._getAction(true, "pencil", "", (evt) => this._openSelection(cur, evt), false, "actions-end"), this._getAction(true, "x", "", (evt) => this._deleteSelection(i, evt), false, "actions-end"))));
            }
            return prev;
        }, []))));
    }
    ...
    _getSelectionSetIds(selectionSet) {
        return selectionSet.workflowType !== interfaces.EWorkflowType.REFINE ? selectionSet.selectedIds :
    ...
    async _getNumDuplicates() {
        ...
        const labels = await downloadUtils.consolidateLabels(this.mapView.map, exportInfos);
        const duplicatesRemoved = downloadUtils.removeDuplicateLabels(labels);
    ...
    _getRefinePage() {
        ...
        return (index.h("calcite-panel", null, this._getLabel(this._translations.refineSelection), hasSelections ? (index.h("div", null, this._getNotice(this._translations.refineTip, "padding-sides-1"), index.h("refine-selection", { enabledLayerIds: this.selectionLayerIds, locale: this.locale, mapView: this.mapView, selectionSets: this._selectionSets, sketchLineSymbol: this.sketchLineSymbol, sketchPointSymbol: this.sketchPointSymbol, sketchPolygonSymbol: this.sketchPolygonSymbol }))) :
    ...
    _getExportSelectionLists() {
        return this._selectionSets.reduce((prev, cur) => {
            const ids = this._getSelectionSetIds(cur);
            const validSet = cur.workflowType !== interfaces.EWorkflowType.REFINE || ids.length > 0;
            if (!this._downloadActive && cur.download && validSet) {
                this._downloadActive = true;
            }
            if (validSet) {
                prev.push((index.h("div", { class: "display-flex padding-sides-1 padding-bottom-1" }, index.h("calcite-checkbox", { checked: cur.download, class: "align-center", onClick: () => { void this._toggleDownload(cur.id); } }), index.h("calcite-list", { class: "list-border margin-start-1-2 width-full", id: "download-list" }, index.h("calcite-list-item", { disabled: !cur.download, label: cur.label, onClick: () => { void this._toggleDownload(cur.id); } }, index.h("div", { slot: "content" }, index.h("div", { class: "list-label" }, cur.label), index.h("div", { class: "list-description" }, cur?.layerView?.layer.title), index.h("div", { class: "list-description" }, this._translations.selectedFeatures.replace("{{n}}", ids.length.toString()))))))));
            }
            return prev;
        }, []) || (index.h("div", null));
    }
    ...
    */
    ...
    _getAction(enabled, icon, text, onClick, indicator = false, slot = "") {
        return (index.h("calcite-action", { disabled: !enabled, icon: icon, iconFlipRtl: true, indicator: indicator, onClick: onClick, slot: slot, text: text }));
    }
    /**
    ...
    _openSelection(selectionSet, evt) {
        ...
        this._pageType = selectionSet.workflowType === interfaces.EWorkflowType.REFINE ?
            interfaces.EPageType.REFINE : interfaces.EPageType.SELECT;
    }
    /**
    ...
    async _highlightFeatures() {
        this._clearHighlight();
        const idSets = this._getSelectionIdsAndViews(this._selectionSets, this._pageType === interfaces.EPageType.EXPORT);
...
PublicNotification.style = PublicNotificationStyle0;
