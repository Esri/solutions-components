...
import { v as validateInteractivity, g as getTheme } from './helpers-e8852b4a.js';
...
const InstantAppsInteractiveLegendGroupLegendElementCaption = class {
    ...
    render() {
        ...
        return (h("header", { class: `${CSS.interacitveLegendHeader} ${getTheme(this.el)}`, style: {
                borderLeft: '1px solid var(--calcite-color-border-3)',
            } }, h("span", null, h("span", { class: CSS.headerActionContainer }, h("calcite-action", { onClick: this.toggleExpanded(), icon: expanded ? 'chevron-down' : 'chevron-up', appearance: "transparent", text: expandCollapseText, label: expandCollapseText, scale: "s" }), h("h3", { class: `${CSS.header} ${CSS.label}${isChild}` }, (_c = this.activeLayerInfo) === null || _c === void 0 ? void 0 : _c.title)), this.featureCount && isInteractive ? (h("instant-apps-interactive-legend-count", { activeLayerInfo: this.activeLayerInfo, "show-total": true, messages: this.messages, legendvm: this.legendvm })) : null)));
    }
    toggleExpanded() {
        return () => {
...
export { InstantAppsInteractiveLegendGroupLegendElementCaption as instant_apps_interactive_legend_group_legend_element_caption };
