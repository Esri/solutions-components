...
import { g as getTheme, i as interactiveLegendState, q as getCategoriesArray, r as checkNestedUniqueSymbol, t as calculateTotalFeatureCountForNestedSymbols, m as checkRelationshipRamp, w as calculateTotalCount, x as getNestedInfoData, k as getIntLegendLayerData, v as validateInteractivity } from './helpers-e8852b4a.js';
...
const InstantAppsInteractiveLegendLayerElementCaption = class {
    ...
    render() {
        ...
        return (h("header", { class: `${CSS.interacitveLegendHeader} ${getTheme(this.el)}` }, h("calcite-action", { onClick: this.toggleExpanded(), icon: expanded ? 'chevron-down' : 'chevron-up', appearance: "transparent", text: expandCollapseText, label: expandCollapseText, scale: "s" }), h("span", null, h("span", { class: CSS.headerActionContainer }, h("h3", { class: `${CSS.header} ${CSS.label}${isChild}` }, (_c = this.activeLayerInfo) === null || _c === void 0 ? void 0 : _c.title)), this.featureCount && isInteractive ? (h("instant-apps-interactive-legend-count", { activeLayerInfo: this.activeLayerInfo, "show-total": true, messages: this.messages, legendvm: this.legendvm })) : null)));
    }
    toggleExpanded() {
        return () => {
