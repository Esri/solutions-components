...
import { a as getDateTimeFormat, c as connectLocalized, d as disconnectLocalized } from './p-895e7b9c.js';
...
async function getNormalizer(mode) {
    ...
    const { normalize } = await import('./p-7b862835.js');
    return normalize;
}
async function createTimeZoneItems(locale, messages, mode, referenceDate, standardTime) {
    if (mode === "name") {
        const { groupByName } = await import('./p-9f98f213.js');
    ...
    if (mode === "region") {
        const [{ groupByRegion }, { getCountry, global: globalLabel }] = await Promise.all([
            import('./p-8d59545b.js'),
            import('./p-c9f94a25.js'),
        ]);
    ...
    const [{ groupByOffset }, { DateEngine }] = await Promise.all([
        import('./p-c97472e4.js'),
        import('./p-33ff2b9e.js'),
    ]);
